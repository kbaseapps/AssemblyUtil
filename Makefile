...
SERVICE_CAPS = AssemblyUtil
SPEC_FILE = AssemblyUtil.spec
URL = https://kbase.us/services/assemblyutil
DIR = $(shell pwd)
LIB_DIR = lib
SCRIPTS_DIR = scripts
TEST_DIR = test
...
build-test-script:
	...
	echo 'pytest  -vv --cov=$$lib --cov-config=coveragerc --cov-report=html --cov-report=xml .' >> $(TEST_DIR)/$(TEST_SCRIPT_NAME)
